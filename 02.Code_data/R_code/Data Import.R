...
mydata <- data.frame(HABITAT, GST, EYR)
print(mydata)

##append row names to the data frame
row.names(mydata) <- c("Reed", "Pear", "Warn", "Cran", "Lyster", "Red", "Devil", "Olinda")
print(mydata)

plot(mydata$GST, mydata$EYR, type = 'b')



## viewing the data frame in a separate window and allow simple editing. 
## Note you can edit the file, but should only change numeric values. Character changes will cause problems.
fix(mydata)
