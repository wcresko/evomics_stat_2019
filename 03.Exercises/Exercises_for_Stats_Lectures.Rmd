...
## _Exercise 3.1: Examining Binomially Distributed Data
- Using R, simulate the following experimental data 
    - use the `rbinom()` function to create a new data set
    - look at the arguments to see how they relate to the trials, probability of each trial, and number of replicates
...
## _Exercise 3.2: Examining Normally Distributed Data
...
## _Exercise 3.4: Parametric t-test in R_

1. Using R to make a dummy data set that contains one continuous and one categorical value with two levels. To do so draw individuals of the two different factor levels from normal distributions with slightly different means. Take 100 obsv. per factor level. 
...
## _Exercise 3.6: Linear Models to examine the relationship of different hormone and thyroid morphology levels in stickleback fish

Write a script to read in the 'perchlorate.tsv' data set. This data set is of a test of different zebrafish of the toxic compound perchlorate, which interferes with thyroid development and function. Several outcomes of fish thyroid morphology and hormone concentration were measured. The data set comprises the following variables in this order:

- Strain of zebrafish (categorical)
- Perchlorate_Level	(continous)
...
Modify the code below to build a linear model of two continuous variables. Notice how the output of the linear model is specified to a new variable. Also note that the variables and dataset are placeholders
...
## _Exercise 4.1: 2-by-2 fixed effect factorial ANOVA examining the effects of microbiome treatment on RNA expression levels in the guts of threespine stickleback
...
3. If there is an interaction between the two factors, can we understand it by looking at the boxplots?
...
## _Exercise 4.2: NMDS for the analysis of multivariate data

Nonmetric Multidimensional Scaling (NMDS) is a form of nonparametric clustering. As such, some of the distributional assumptions of multivariate analyses are relaxed. The package VEGAN was orginally derived for ecological data across plots or sites. For example, ennumeration of species across different locations. However, the approach is very applicable to genetic data across geographic locals or phenotypes, or gene expression data across different samples.

Now examine the 'yeast.tsv' data set, which is a measurement of RNAseq levels of numerous genes from different six different yeast samples from different vineyards.
...
Let's take a look at how the dissimilarities among samples maps onto the ordination distance. Notice that there is a fit with the data, but we're no longer assumed consistent linearity over the entire data set.
...
Lastly, we can use the dissimilarity matrices to perform hierarchical clustering. Try both the non-normalized (clus.dis1) and normalized (clus.dis2) distances.
...
## _Exercise 4.3: Biodiversity analysis in R - pulling it all together
