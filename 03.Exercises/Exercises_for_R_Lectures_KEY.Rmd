...
```{r}
rna <- read.csv('data/RNAseq_sample_data.csv', header=T, sep=',')
str(rna)
head(rna)
```
...
```{r, echo=T, eval=F}
mutate(sub,sqrt(Gene1), sqrt(Gene2),...)
```
...
```{r, echo=T, eval=F}
by_geno <- group_by(sub, Genotype, Microbiota)
summarise(by_geno, g1mean = mean(Gene1, na.rm = TRUE),g2mean = mean(Gene2, na.rm = TRUE),...)
```
...
```{r, echo=T, eval=F}
boxplot(sub$Gene1~sub$Microbiota)
boxplot(sqrt(sub$Gene1)~sub$Microbiota)
```
